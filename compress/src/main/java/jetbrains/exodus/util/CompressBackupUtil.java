...
public class CompressBackupUtil {
    ...
    public static TarArchiveInputStream reEncryptBackup(final ArchiveInputStream archiveStream,
                                                        final byte @Nullable [] firstKey, final long firstIv,
                                                        final byte @Nullable [] secondKey, final long secondIv,
                                                        final @Nullable StreamCipherProvider cipherProvider)
            throws IOException {
    ...
    private static void encryptV2FormatPages(byte @NotNull [] key, long iv,
                                             @NotNull StreamCipherProvider cipherProvider,
                                             byte @NotNull [] readBuffer, int pageSize, int processed, long fileAddress,
                                             byte @NotNull [] dataToWrite, int dataToWriteLen) {
        for (int dataOffset = 0; dataOffset < dataToWriteLen; dataOffset += pageSize) {
