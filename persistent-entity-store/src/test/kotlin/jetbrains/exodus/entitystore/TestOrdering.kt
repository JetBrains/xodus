...
class TestOrdering : EntityStoreTestBase() {
    ...
    fun testDirectOrder() {
        ...
        for ((i, e) in txn.getAll("Issue").withIndex()) {
            val id = e.id
            TestCase.assertEquals(0, id.typeId)
            TestCase.assertEquals(i.toLong(), id.localId)
        }
    }
    ...
    fun testBindings() {
        ...
        Assert.assertTrue(
            Arrays.compareUnsigned(entry0.bytesUnsafe, 0, entry0.length,
        ...
        Assert.assertTrue(
            Arrays.compareUnsigned(entry1.bytesUnsafe, 0, entry1.length,
                entry2.bytesUnsafe, 0, entry2.length) < 0)
        Assert.assertTrue(
            Arrays.compareUnsigned(entry1.bytesUnsafe, 0, entry1.length,
                entry3.bytesUnsafe, 0, entry3.length) < 0)
        ...
        Assert.assertTrue(
            Arrays.compareUnsigned(entry2.bytesUnsafe, 0, entry2.length,
                entry3.bytesUnsafe, 0, entry3.length) < 0)
        Assert.assertTrue(
            Arrays.compareUnsigned(entry2.bytesUnsafe, 0, entry2.length,
                entry1.bytesUnsafe, 0, entry1.length) > 0)
