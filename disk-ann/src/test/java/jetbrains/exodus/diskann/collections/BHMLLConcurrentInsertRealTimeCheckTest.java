...
public class BHMLLConcurrentInsertRealTimeCheckTest {
    ...
    public void testConcurrentInsertRealTimeCheck() {
        ...
        try (var executor = Executors.newCachedThreadPool()) {
            ...
            for (var i = 0; i < THREAD_CHECK_COUNT; i++) {
                removeFutures[i] = executor.submit(new NBHMLLConcurrentCheckRunnable(latch, nbhmll, added,
                        lastIteration));
            }
            ...
            for (long i = 0; i < THREAD_INSERTION_COUNT * INSERT_COUNT; i++) {
                Assert.assertEquals("key  = " + i, 2 * i, nbhmll.get(i));
            }
        }
    }
    ...
    private static final class NBHMLLConcurrentCheckRunnable implements Callable<Void> {
        private final CountDownLatch latch;
        ...
        private NBHMLLConcurrentCheckRunnable(CountDownLatch latch,
                                              NonBlockingHashMapLongLong nbhmll,
        ...
        public Void call() {
            ...
            do {
                for (var entry : added.entrySet()) {
                    var value = nbhmll.get(entry.getKey());
                    Assert.assertEquals("key  = " + entry.getKey(), entry.getValue().longValue(), value);
