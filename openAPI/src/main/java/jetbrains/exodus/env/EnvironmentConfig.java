...
/**
 * Specifies settings of {@linkplain Environment}. Default settings are specified by {@linkplain #DEFAULT} which is
 * immutable. Any newly created {@code EnvironmentConfig} has the same settings as {@linkplain #DEFAULT}.
 *
 * <p>As a rule, the {@code EnvironmentConfig} instance is created along with the
 * {@linkplain Environment} one. E.g., creation of {@linkplain Environment} with some tuned garbage collector settings
 * can look as follows:
 * <pre>
 ...
 * E.g., {@linkplain #LOG_FILE_SIZE} defines the size of a single {@code Log} file (.xd file) which cannot be changed
 * for existing databases. It makes sense to choose values of such settings at design time and hardcode them.
 *
...
public class EnvironmentConfig extends AbstractConfig {
  ...
  /**
   * Defines percent of max memory (specified by the "-Xmx" java parameter) that can be used by the LogCache. Is
   * applicable only if {@linkplain #MEMORY_USAGE} is not set. Default value is {@code 50}.
   * <p>Mutable at runtime: no
   *
   * @see #MEMORY_USAGE
   */
  ...
  /**
   * Xodus performs check of consistency of pages loaded from disk. This option allows to enable/disable this check.
   * <p>
   * Disabling this check should improve general performance of database but decreases durability guaranties.
   * <p>
   ...
   * {@code null}, which means that the database won't be encrypted. The setting cannot be changed for existing
   * databases. Default value is {@code null}.
   * <p>Mutable at runtime: no
   *
   * @see #CIPHER_KEY
   * @see #CIPHER_BASIC_IV
  ...
  /**
   * Defines the key which will be used to encrypt the database. The key is expected to be a hex string representing a
   * byte array which is passed to {@linkplain StreamCipher#init(byte[], long)}. Is applicable only if
   * {@linkplain #CIPHER_ID} is not {@code null}. The setting cannot be changed for existing databases. Default value is
   * {@code null}.
   * <p>Mutable at runtime: no
   *
   * @see #CIPHER_ID
   ...
   * The setting cannot be changed for existing databases. Default value is {@code 0L}.
   * <p>Mutable at runtime: no
   *
   * @see #CIPHER_ID
   * @see #CIPHER_KEY
  ...
  /**
   * Defines the maximum size in kilobytes of a single {@code Log} file (.xd file). The setting cannot be changed for
   * existing databases. Default value is {@code 8192L}.
   * <p>Mutable at runtime: no
   */
  ...
  /**
   * Defines the debug identifier to be written to the lock file alongside with other debug information. Default value
   * is {@code ManagementFactory.getRuntimeMXBean().getName()} which has a form of {@code pid@hostname}.
   * <p>Mutable at runtime: no
   */
   ...
   * should be configured to use single LogCache page size.
   *
  ...
  /**
   * Defines the maximum number of open files that LogCache maintains in order to reduce system calls to open and close
   * files. The more open files is allowed the less system calls are performed in addition to reading or mapping files.
   * This value can notably affect performance of database warm-up. Default value is {@code 500}. Open files cache is
   * shared amongst all open {@linkplain Environment environments}.
   * <p>Mutable at runtime: no
   */
  public static final String LOG_CACHE_OPEN_FILES = "exodus.log.cache.openFilesCount";

  /**
   * If is set to {@code true} any immutable file can be mapped in memory provided there is enough physical memory. On
   * cache miss, LogCache at first tries to check if there is corresponding mapped byte buffer and copies cache page
   * from the buffer, otherwise reads the page from {@linkplain java.io.RandomAccessFile}. If is set to {@code false}
   * then LogCache always reads {@linkplain java.io.RandomAccessFile} on cache miss. Default value was {@code true}
   * before version {@code 1.2.3}. As of {@code 1.2.3}, default value is {@code false}.
   *
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * If is set to {@code true} the LogCache is shared. Shared cache caches raw binary data (contents of .xd files) of
   * all {@linkplain Environment} instances created in scope of this class loader. By default, the LogCache is shared.
   * <p>Mutable at runtime: no
   */
   ...
   * {@linkplain #LOG_CACHE_NON_BLOCKING} is set to {@code true}. The higher number of generations is, the higher the
   * cache hit rate is and CPU ticks necessary to get a single page from the cache. Default value is {@code 2}.
   * <p>Mutable at runtime: no
   */
  public static final String LOG_CACHE_GENERATION_COUNT = "exodus.log.cache.generationCount";

  /**
   * If is set to {@code true} LogCache uses {@linkplain java.lang.ref.SoftReference soft references} for holding cached
   * pages. The cache still uses not more memory than it is configured by {@linkplain #MEMORY_USAGE} or
   * {@linkplain #MEMORY_USAGE_PERCENTAGE} settings, but JVM GC can reclaim memory used by the cache on a heavy load
   * surge. On the other hand, use of soft references results in greater JVM GC load and greater general CPU consumption
   * by the cache. So one can choose either memory-flexible, or CPU-optimal cache. Default value is {@code false}.
   * <p>Mutable at runtime: no
   */
  public static final String LOG_CACHE_USE_SOFT_REFERENCES = "exodus.log.cache.useSoftReferences";

  /**
   * Defines the number of successive pages to be read at once in case of LogCache miss. Reading successive pages can
   * reduce amount of random access to database files. It can be useful in workloads like application warm-up. Default
   * value is {@code 1} which means that no read-ahead strategy is applied.
   * <p>Mutable at runtime: yes
   */
  public static final String LOG_CACHE_READ_AHEAD_MULTIPLE = "exodus.log.cache.readAheadMultiple";

  /**
   * If is set to {@code true} LogCache will populate itself with database file pages right after the database is opened
   * using this {@code EnvironmentConfig} instance. Default value is {@code false}.
   * <p>Mutable at runtime: no
   */
  public static final String LOG_CACHE_WARMUP = "exodus.log.cache.warmup";

  /**
   * If is set to {@code true} then the Log constructor fails if the database directory is not clean. Can be useful if
   * an applications expects that the database should always be newly created. Default value is {@code false}.
   * <p>Mutable at runtime: no
   */
  public static final String LOG_CLEAN_DIRECTORY_EXPECTED = "exodus.log.cleanDirectoryExpected";

  /**
   * If is set to {@code true} then the Log constructor implicitly clears the database if it occurred to be invalid when
   * opened. Default value is {@code false}.
   * <p>Mutable at runtime: no
   */
  ...
  /**
   * Sets the period in milliseconds to force file system's fsync call that often if {@linkplain #LOG_DURABLE_WRITE} is
   * switched off. Default value is {@code 10000L}.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * Forces data restore routine to proceed even if it is not possible to restore all the data. Default value is
   * {@code false}.
   *
   * <p>Mutable at runtime: no
   */
   ...
   * {@linkplain DataReaderWriterProvider} service provider interface implementation, if is set to {@code true} then the
   * database can be opened on a removable storage. Attempt to open database on a storage of not allowed type results in
   * {@linkplain StorageTypeNotAllowedException}. Default value is {@code false}.
   * <p>Mutable at runtime: no
   *
   * @since 1.4.0
   */
   ...
   * For {@linkplain DataReaderWriterProvider#DEFAULT_READER_WRITER_PROVIDER} used as
   * {@linkplain DataReaderWriterProvider} service provider interface implementation, if is set to {@code true} then the
   * database can be opened on a remote storage. Attempt to open database on a storage of not allowed type results in
   * {@linkplain StorageTypeNotAllowedException}. Default value is {@code false}.
   * <p>Mutable at runtime: no
   *
   * @since 1.4.0
   ...
   * For {@linkplain DataReaderWriterProvider#DEFAULT_READER_WRITER_PROVIDER} used as
   * {@linkplain DataReaderWriterProvider} service provider interface implementation, if is set to {@code true} then the
   * database can be opened on RAM-disk. Attempt to open database on a storage of not allowed type results in
   * {@linkplain StorageTypeNotAllowedException}. Default value is {@code false}.
   * <p>Mutable at runtime: no
   *
   * @since 1.4.0
  ...
  /**
   * Defines fully-qualified name of the {@linkplain DataReaderWriterProvider} service provider interface implementation
   * which will be used to create {@linkplain DataReader} and {@linkplain DataWriter} instances. This setting can be
   * used to customize storageL define in-memory one, in-cloud, etc. Default value is
   * {@linkplain DataReaderWriterProvider#DEFAULT_READER_WRITER_PROVIDER} which means that file system must be used as a
   * storage. Several settings are applicable only to FileDataReaderWriterProvider used:
   ...
   * {@linkplain Store#get(Transaction, ByteIterable)} in certain cases. Default value is {@code 0} what means that the
   * cache is inactive. If the setting is mutated at runtime the cache is invalidated.
   * <p>Mutable at runtime: yes
   */
  ...
  /**
   * If is set to {@code true} then {@linkplain Environment#close()} doest't check if there are unfinished transactions.
   * Otherwise it checks and throws {@linkplain ExodusException} if there are. Default value is {@code false}.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * Enables printing the "environment created" message. {@code true} by default, but setting it to {@code false} can be
   * useful in tests that create many Environment instances if we don't want to get flooded by redundant log messages.
   */
  public static final String ENV_PRINT_INIT_MESSAGE = "exodus.env.skipInitMessage";

  /**
   * If is set to {@code true} then {@linkplain Environment} performs check of consistency of datastructures stored in
   * backup files. Default value is {@code false}. Mutable at runtime: no
   */
  ...
  /**
   * If is set to {@code true} then {@code jetbrains.exodus.entitystore.StoreTransaction#findContaining} starts using
   * tailored underlying implementation that yields twice throughput of the default one, but sacrifices precision in
   * operation with some Unicode specimen, namely Turkish dotted/dotless I. Mutable at runtime: yes Default: false
   */
  public static final String ENV_TXN_QUERY_OPTIMIZED_CONTAINS = "exodus.env.txn.optimizedContains";

  /**
   * Defines the number of times which a {@linkplain Transaction} can try to flush without attempts to upgrade (switch
   * to an exclusive mode). Default value is {@code 2}.
   * <p>Mutable at runtime: yes
   *
   * @see Transaction
  ...
  /**
   * If is set to {@code true} then any write operation can be performed only in the thread which the transaction was
   * created in. Default value is {@code false}.
   * <p>Mutable at runtime: yes
   *
   * @see Transaction
   */
  ...
  /**
   * If is set to {@code true} then each transaction, read/write or read-only, saves stack trace when it is finished
   * (aborted or committed). The stack trace is then reported with {@code TransactionFinishedException}. Default value
   * is {@code false}.
   * <p>Mutable at runtime: yes
   *
   * @see Transaction
  ...
  public static final String ENV_TRANSACTION_PERMISSION_TIMEOUT = "exodus.env.txn.timeout";

  /**
   * Defines the number of {@linkplain Transaction transactions} that can be started in parallel. It is unlimited by
   * default.
   * <p>Mutable at runtime: no
   *
   * @see Transaction
  ...
  /**
   * Defines the number of read-only {@linkplain Transaction transactions} that can be started in parallel. By default
   * it is unlimited.
   * <p>Mutable at runtime: no
   * <p>
  ...
  /**
   * Defines {@linkplain Transaction} timeout in milliseconds. If transaction doesn't finish in this timeout then it is
   * reported in logs as stuck along with stack trace which it was created with. Default value is {@code 0} which means
   * that no timeout for a {@linkplain Transaction} is defined. In that case, no monitor of stuck transactions is
   * started. Otherwise it is started for each {@linkplain Environment}, though consuming only a single
  ...
  /**
   * Defines {@linkplain Transaction} expiration timeout in milliseconds. If transaction doesn't finish in this timeout
   * then it is forced to be finished. Default value is {@code 8} hours. {@code 0} value means that no expiration for a
   * {@linkplain Transaction} is defined. In that case, no monitor of stuck transactions is started. Otherwise it is
   * started for each {@linkplain Environment}, though consuming only a single {@linkplain Thread} amongst all
   ...
   * {@linkplain Environment}'s transactions with this frequency (period) specified in milliseconds. Default value is
   * {@code 60000}, one minute.
   * <p>Mutable at runtime: no
   *
   * @see Transaction
  ...
  /**
   * If is set to {@code true} then the {@linkplain Environment} gathers statistics. If {@linkplain #MANAGEMENT_ENABLED}
   * is also {@code true} then the statistics is exposed by the JMX managed bean. Default value is {@code true}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * If is set to {@code true} then the {@linkplain Environment} will compact itself on opening. Default value is
   * {@code false}.
   * <p>Mutable at runtime: no
   */
  ...
  /**
   * If is set to {@code true} then the database garbage collector is enabled. Default value is {@code true}. Switching
   * GC off makes sense only for debugging and troubleshooting purposes.
   * <p>Mutable at runtime: yes
   */
  ...
  /**
   * Defines percent of minimum database utilization. Default value is {@code 50}. That means that 50 percent of free
   * space in raw data in {@code Log} files (.xd files) is allowed. If database utilization is less than defined (free
   * space percent is more than {@code 50}), the database garbage collector is triggered.
   * <p>Mutable at runtime: yes
   */
  public static final String GC_MIN_UTILIZATION = "exodus.gc.minUtilization";

  /**
   * If is set to {@code true} the database garbage collector renames files rather than deletes them. Default value is
   * {@code false}. It makes sense to change this setting only for debugging and troubleshooting purposes.
   * <p>Mutable at runtime: yes
   */
   ...
   * collector. The age of the last (the newest, the rightmost) {@code Log} file is {@code 0}, the age of previous file
   * is {@code 1}, etc. Default value is {@code 2}.
   * <p>Mutable at runtime: yes
   */
   ...
   * collector) can be repeated if the previous one didn't reach required utilization. Default value is {@code 5000}.
   * <p>Mutable at runtime: yes
   */
  public static final String GC_RUN_PERIOD = "exodus.gc.runPeriod";

  /**
   * If is set to {@code true} then database utilization will be computed from scratch before the first cleaning cycle
   * (single run of the database garbage collector) is triggered, i.e. shortly after the database is open. In addition,
   * can be used to compute utilization information at runtime by just modifying the setting value. Default value is
   * {@code false}.
   * <p>Mutable at runtime: yes
   */
   ...
   * {@linkplain Environment} to update {@code .xd} files' utilization before the first cleaning cycle (single run of
   * the database garbage collector) is triggered. In addition, can be used to reload utilization information at runtime
   * by just modifying the setting value. Format of the stored utilization is expected to be the same as created by the
   * {@code "-d"} option of the {@code Reflect} tool. Default value is empty string.
   * <p>Mutable at runtime: yes
   */
  public static final String GC_UTILIZATION_FROM_FILE = "exodus.gc.utilization.fromFile";

  /**
   * If is set to {@code true} the database garbage collector tries to acquire exclusive {@linkplain Transaction} for
   * its purposes. In that case, GC transaction never re-plays. In order to not block background cleaner thread forever,
   * acquisition of exclusive GC transaction is performed with a timeout controlled by the
   ...
   * {@linkplain Transaction} for its purposes if {@linkplain #GC_USE_EXCLUSIVE_TRANSACTION} is {@code true}. Default
   * value is {@code 1000}.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * Defines timeout in milliseconds which is used by the database garbage collector to reclaim non-expired data in
   * several files inside single GC {@linkplain Transaction} acquired exclusively.
   * {@linkplain #GC_USE_EXCLUSIVE_TRANSACTION} should be {@code true}. Default value is {@code 500}.
   * <p>Mutable at runtime: yes
   *
   * @see #GC_USE_EXCLUSIVE_TRANSACTION
   * @see #GC_TRANSACTION_ACQUIRE_TIMEOUT
  ...
  /**
   * Defines the number of milliseconds which deletion of any successfully cleaned {@code Log} file (.xd file) is
   * postponed for. Default value is {@code 5000}.
   * <p>Mutable at runtime: yes
   */
  public static final String GC_FILES_DELETION_DELAY = "exodus.gc.filesDeletionDelay";

  /**
   * If set to nonzero value, GC is forced every this number of seconds. Default value is {@code 0} which means that GC
   * is not forced periodically.
   * <p>Mutable at runtime: yes
   */
   ...
   * {@code EnvironmentConfig} settings. Default value is {@code true} for non-Android OS, under Android it is always
   * {@code false}.
   * <p>Mutable at runtime: no
   *
   * @see Environment#getStatistics()
  ...
  /**
   * If is set to {@code true} then exposed JMX managed beans cannot have operations. Default value is {@code true}.
   * <p>Mutable at runtime: no
   *
   * @see #MANAGEMENT_ENABLED
   */
  ...
  /**
   * Maximum size of a single tree entry. Default value is half of maximum of segment size.
   */
  public static final String ENV_MAXIMUM_TREE_ENTRY_SIZE = "exodus.env.maximumTreeEntrySize";
  ...
  public EnvironmentConfig(@NotNull final ConfigurationStrategy strategy) {
    ...
    super(new Pair[]{
        ...
        new Pair(ENV_MAXIMUM_TREE_ENTRY_SIZE, 4 * 1024 * 1024),
        new Pair(ENV_TRANSACTION_PERMISSION_TIMEOUT, 10)
    }, strategy);
  }
  ...
  /**
   * Sets absolute value of memory in bytes that can be used by the LogCache. Overrides memory percent to use by the
   * LogCache set by {@linkplain #setMemoryUsagePercentage(int)}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Returns percent of max memory (specified by the "-Xmx" java parameter) that can be used by the LogCache. Is
   * applicable only if {@linkplain #MEMORY_USAGE} is not set. Default value is {@code 50}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Sets percent of max memory (specified by the "-Xmx" java parameter) that can be used by the LogCache. Is applicable
   * only if {@linkplain #MEMORY_USAGE} is not set. Default value is {@code 50}.
   * <p>Mutable at runtime: no
   *
   ...
   *
   * @param checkPagesAtRuntime {@code ture} if consistency of disk pages is checked during runtime and {@code false}
   *                            otherwise.
   * @see #CHECK_PAGES_AT_RUNTIME
   */
  ...
  /**
   * Returns the key which will be used to encrypt the database or {@code null} for no encryption. Is applicable only
   * if* {@linkplain #getCipherId()} returns not {@code null}. Default value is {@code null}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Sets the key which will be used to encrypt the database. The key is expected to be a hex string representing a byte
   * array which is passed to {@linkplain StreamCipher#init(byte[], long)}. Is applicable only if
   * {@linkplain #getCipherId()} returns not {@code null}. The setting cannot be changed for existing databases. Default
   * value is {@code null}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Sets basic IV (initialization vector) which will be used to encrypt the database. Basic IV is expected to be random
   * (pseudo-random) and unique long value. Basic IV is used to calculate relative IVs which are passed to
   * {@linkplain StreamCipher#init(byte[], long)}. Is applicable only if {@linkplain #CIPHER_ID} is not {@code null}.
   * The setting cannot be changed for existing databases. Default value is {@code 0L}.
   * <p>Mutable at runtime: no
   *
   ...
   * transaction. By default, is switched off since it creates significant performance overhead and can be controlled
   * manually.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * Sets flag whether {@linkplain Transaction transactions} should force fsync after each commit or flush. By default,
   * is switched off since it creates significant performance overhead and can be controlled manually.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * Returns the maximum size in kilobytes of a single {@code Log} file (.xd file). The setting cannot be changed for
   * existing databases. Default value is {@code 8192L}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Sets the maximum size in kilobytes of a single {@code Log} file (.xd file). The setting cannot be changed for
   * existing databases. Default value is {@code 8192L}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Returns the number of milliseconds the {@code Log} constructor waits for the lock file. Default value is
   * {@code 0L}, i.e. it doesn't wait and fails immediately if the lock is acquired.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Sets the debug identifier to be written to the lock file alongside with other debug information. Default value is
   * {@code ManagementFactory.getRuntimeMXBean().getName()} which has a form of {@code pid@hostname}.
   * <p>Mutable at runtime: no
   *
   * @return the debug identifier to be written to the lock file alongside with other debug information or null if the
   ...
   * close files. The more open files is allowed the less system calls are performed in addition to reading or mapping
   * files. This value can notably affect performance of database warm-up. Default value is {@code 500}. Open files
   * cache is shared amongst all open {@linkplain Environment environments}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Sets the maximum number of open files that the LogCache maintains in order to reduce system calls to open and close
   * files. The more open files is allowed the less system calls are performed in addition to reading or mapping files.
   * This value can notably affect performance of database warm-up. Default value is {@code 500}. Open files cache is
   * shared amongst all open {@linkplain Environment environments}.
   * <p>Mutable at runtime: no
  ...
  /**
   * Returns {@code true} if any immutable .xd file can be mapped in memory provided there is enough physical memory. On
   * cache miss, LogCache at first tries to check if there is corresponding mapped byte buffer and copies cache page
   * from the buffer, otherwise reads the page from {@linkplain java.io.RandomAccessFile}. If is set to {@code false}
   * then LogCache always reads {@linkplain java.io.RandomAccessFile} on cache miss. Default value was {@code true}
   * before version {@code 1.2.3}. As of {@code 1.2.3}, default value is {@code false}.
   ...
   * On cache miss, LogCache at first tries to check if there is corresponding mapped byte buffer and copies cache page
   * from the buffer, otherwise reads the page from {@linkplain java.io.RandomAccessFile}. If is set to {@code false}
   * then LogCache always reads {@linkplain java.io.RandomAccessFile} on cache miss. Default value was {@code true}
   * before version {@code 1.2.3}. As of {@code 1.2.3}, default value is {@code false}.
   *
  ...
  /**
   * Returns {@code true} if the LogCache is shared. Shared cache caches raw binary data (contents of .xd files) of all
   * {@linkplain Environment} instances created in scope of this class loader. By default, the LogCache is shared.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Set {@code true} if the LogCache should be shared. Shared cache caches raw binary data (contents of .xd files) of
   * all {@linkplain Environment} instances created in scope of this class loader. By default, the LogCache is shared.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Returns {@code true} if the LogCache should use lock-free data structures. Default value is {@code true}. There is
   * no practical sense to use "blocking" cache, so the setting will be deprecated in future.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Set {@code true} if the LogCache should use lock-free data structures. Default value is {@code true}. There is no
   * practical sense to use "blocking" cache, so the setting will be deprecated in future.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Returns {@code true} LogCache uses {@linkplain java.lang.ref.SoftReference soft references} for holding cached
   * pages. The cache still uses not more memory than it is configured by {@linkplain #MEMORY_USAGE} or
   * {@linkplain #MEMORY_USAGE_PERCENTAGE} settings, but JVM GC can reclaim memory used by the cache on a heavy load
   * surge. On the other hand, use of soft references results in greater JVM GC load and greater general CPU consumption
   * by the cache. So one can choose either memory-flexible, or CPU-optimal cache. Default value is {@code false}.
   * <p>Mutable at runtime: no
   *
   * @return {@code true} if LogCache should use {@linkplain java.lang.ref.SoftReference soft references} for holding
   * cached pages
   */
  ...
  /**
   * Set {@code true} if LogCache should use {@linkplain java.lang.ref.SoftReference soft references} for holding cached
   * pages. The cache still uses not more memory than it is configured by {@linkplain #MEMORY_USAGE} or
   * {@linkplain #MEMORY_USAGE_PERCENTAGE} settings, but JVM GC can reclaim memory used by the cache on a heavy load
   * surge. On the other hand, use of soft references results in greater JVM GC load and greater general CPU consumption
   * by the cache. So one can choose either memory-flexible, or CPU-optimal cache. Default value is {@code false}.
  ...
  /**
   * Returns the number of successive pages to be read at once in case of LogCache miss. Reading successive pages can
   * reduce amount of random access to database files. It can be useful in workloads like application warm-up. Default
   * value is {@code 1} which means that no read-ahead strategy is applied.
   *
  ...
  /**
   * Sets the number of successive pages to be read at once in case of LogCache miss. Reading successive pages can
   * reduce amount of random access to database files. It can be useful in workloads like application warm-up. Default
   * value is {@code 1} which means that no read-ahead strategy is applied.
   *
  ...
  /**
   * Returns {@code true} if LogCache will populate itself with database file pages right after the database is opened
   * using this {@code EnvironmentConfig} instance. Default value is {@code false}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Set {@code true} if LogCache should populate itself with database file pages right after the database is opened
   * using this {@code EnvironmentConfig} instance. Default value is {@code false}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Returns {@code true} if the Log constructor fails if the database directory is not clean. Can be useful if an
   * applications expects that the database should always be newly created. Default value is {@code false}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Set {@code true} if the Log constructor should fail if the database directory is not clean. Can be useful if an
   * applications expects that the database should always be newly created. Default value is {@code false}.
   * <p>Mutable at runtime: no
   *
   * @param logCleanDirectoryExpected {@code true} if the Log constructor should fail if the database directory is not
   *                                  clean
   * @return this {@code EnvironmentConfig} instance
   */
  ...
  /**
   * Returns {@code true} if the Log constructor implicitly clears the database if it occurred to be invalid when
   * opened. Default value is {@code false}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Set {@code true} if the Log constructor should implicitly clear the database if it occurred to be invalid when
   * opened. Default value is {@code false}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Set {@code true} if each complete and immutable {@code Log} file (.xd file) should marked with read-only attribute.
   * Default value is {@code true}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Returns fully-qualified name of the {@linkplain DataReaderWriterProvider} service provide interface implementation
   * which will be used to create {@linkplain DataReader} and {@linkplain DataWriter} instances. This setting can be
   * used to customize storage: define in-memory one, in-cloud, etc. Default value is
   * {@linkplain DataReaderWriterProvider#DEFAULT_READER_WRITER_PROVIDER} which means that file system must be used as a
   * storage. Several settings are applicable only if FileDataReaderWriterProvider is used:
   * {@linkplain #LOG_DURABLE_WRITE}, {@linkplain #LOG_SYNC_PERIOD}, {@linkplain #LOG_CACHE_OPEN_FILES},
   * {@linkplain #LOG_FULL_FILE_READ_ONLY}, {@linkplain #LOG_CACHE_USE_NIO},
   * {@linkplain #LOG_CACHE_FREE_PHYSICAL_MEMORY_THRESHOLD}.
   * <p>Mutable at runtime: no
  ...
  /**
   * Sets fully-qualified name of the {@linkplain DataReaderWriterProvider} service provide interface implementation
   * which will be used to create {@linkplain DataReader} and {@linkplain DataWriter} instances. This setting can be
   * used to customize storage: define in-memory one, in-cloud, etc. Default value is
   * {@linkplain DataReaderWriterProvider#DEFAULT_READER_WRITER_PROVIDER} which means that file system must be used as a
   * storage. Several settings are applicable only if FileDataReaderWriterProvider is used:
   ...
   * <p>Mutable at runtime: no
   *
   * @param provider fully-qualified name of the {@linkplain DataReaderWriterProvider} service provide interface
   *                 implementation
   * @return this {@code EnvironmentConfig} instance
   */
   ...
   * <p>Mutable at runtime: yes
   *
   * @param readonlyEmptyStores {@code true} if attempt to create a {@linkplain Store} shouldn't fail immediately in
   *                            read-only mode
   * @return this {@code EnvironmentConfig} instance
   */
  ...
  /**
   * Returns {@code true} if {@linkplain Environment#close()} shouldn't check if there are unfinished transactions.
   * Otherwise it should check and throw {@linkplain ExodusException} if there are. Default value is {@code false}.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * Set {@code true} if {@linkplain Environment#close()} shouldn't check if there are unfinished transactions. Set
   * {@code false} if it should check and throw {@linkplain ExodusException} if there are unfinished transactions.
   ...
   * Maximum size of a single tree entry. Default value is half of maximum of segment size.
   *
   * @param size Maximum size of a single tree entry.
   * @return this {@code EnvironmentConfig} instance
   */
  public EnvironmentConfig setEnvMaximumTreeEntrySize(int size) {
    return setSetting(ENV_MAXIMUM_TREE_ENTRY_SIZE, size);
  }

  /**
   * Maximum size of a single tree entry. Default value is half of maximum of segment size.
   *
   * @return Maximum size of a single tree entry.
   */
  public int getEnvMaximumTreeEntrySize() {
    return (Integer) getSetting(ENV_MAXIMUM_TREE_ENTRY_SIZE);
  }

  /**
   * If is set to {@code true} then {@linkplain Environment} performs check of consistency of datastructures stored in
   * backup files. Default value is {@code false}.
   * <p>Mutable at runtime: no
   */
  ...
  /**
   * Returns the number of times which a {@linkplain Transaction} can try to flush without attempts to upgrade (switch
   * to an exclusive mode). Default value is {@code 2}.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * Returns whether to use optimized contains implementation for `propertyContains` request, that yields higher
   * throughput, but fails on some Unicode specimen.
   *
  ...
  /**
   * Sets the number of times which a {@linkplain Transaction} can try to flush without attempts to upgrade (switch to
   * an exclusive mode). Default value is {@code 2}.
   * <p>Mutable at runtime: yes
   *
   ...
   * <p>Mutable at runtime: yes
   *
   * @return {@code true} if any write operation can be performed only in the thread which the transaction was created
   * in
   * @see Transaction
   */
   ...
   * <p>Mutable at runtime: yes
   *
   * @param singleThreadWrites {@code true} then any write operation can be performed only in the thread which the
  ...
  /**
   * Returns the number of read-only {@linkplain Transaction transactions} that can be started in parallel. By default
   * it is unlimited.
   * <p>Mutable at runtime: no
   * <p>
   * As of 1.4.0, is deprecated.
  ...
  /**
   * Sets the number of read-only {@linkplain Transaction transactions} that can be started in parallel. By default it
   * is unlimited.
   * <p>Mutable at runtime: no
   * <p>
   * As of 1.4.0, is deprecated.
   *
   * @param maxParallelReadonlyTxns number of read-only {@linkplain Transaction transactions} that can be started in
   *                                parallel
   * @return this {@code EnvironmentConfig} instance
   */
  ...
  /**
   * Returns {@linkplain Transaction} timeout in milliseconds. If transaction doesn't finish in this timeout then it is
   * reported in logs as stuck along with stack trace which it was created with. Default value is {@code 0} which means
   * that no timeout for a {@linkplain Transaction} is defined. In that case, no monitor of stuck transactions is
   * started. Otherwise it is started for each {@linkplain Environment}, though consuming only a single
   * {@linkplain Thread} amongst all environments created within a single class loader.
  ...
  /**
   * Sets {@linkplain Transaction} timeout in milliseconds. If transaction doesn't finish in this timeout then it is
   * reported in logs as stuck along with stack trace which it was created with. Default value is {@code 0} which means
   * that no timeout for a {@linkplain Transaction} is defined. In that case, no monitor of stuck transactions is
   * started. Otherwise it is started for each {@linkplain Environment}, though consuming only a single
   * {@linkplain Thread} amongst all environments created within a single class loader.
   ...
   * Defines {@linkplain Transaction} expiration timeout in milliseconds. If transaction doesn't finish in this timeout
   * then it is forced to be finished. Default value is {@code 8} hours. {@code 0} value means that no expiration for a
   * {@linkplain Transaction} is defined. Otherwise it is started for each {@linkplain Environment}, though consuming
   * only a single {@linkplain Thread} amongst all environments created within a single class loader.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Returns {@code true} if the {@linkplain Environment} gathers statistics. If {@linkplain #MANAGEMENT_ENABLED} is
   * also {@code true} then the statistics is exposed by the JMX managed bean. Default value is {@code true}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Set {@code true} if the {@linkplain Environment} should gather statistics. If {@linkplain #MANAGEMENT_ENABLED} is
   * also {@code true} then the statistics is exposed by the JMX managed bean. Default value is {@code true}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Returns {@code true} if the {@linkplain Environment} will compact itself on opening. Default value is
   * {@code false}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Set {@code true} if the {@linkplain Environment} should compact itself on opening Default value is {@code false}.
   * <p>Mutable at runtime: no
   *
  ...
  /**
   * Sets the maximum size of page of B+Tree. Default value is {@code 128}. Only sizes in the range [16..1024] are
   * accepted.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * Sets the maximum size of page of duplicates sub-B+Tree. Default value is {@code 8}. Only sizes in the range
   * [8..128] are accepted.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * Returns {@code true} if the database garbage collector is enabled. Default value is {@code true}. Switching GC off
   * makes sense only for debugging and troubleshooting purposes.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * Set {@code true} to enable the database garbage collector. Default value is {@code true}. Switching GC off makes
   * sense only for debugging and troubleshooting purposes.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * Returns percent of minimum database utilization. Default value is {@code 50}. That means that 50 percent of free
   * space in raw data in {@code Log} files (.xd files) is allowed. If database utilization is less than defined (free
   * space percent is more than {@code 50}), the database garbage collector is triggered.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * Sets percent of minimum database utilization. Default value is {@code 50}. That means that 50 percent of free space
   * in raw data in {@code Log} files (.xd files) is allowed. If database utilization is less than defined (free space
   * percent is more than {@code 50}), the database garbage collector is triggered.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * If returns {@code true} the database garbage collector renames files rather than deletes them. Default value is
   * {@code false}. It makes sense to change this setting only for debugging and troubleshooting purposes.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * Set {@code true} if the database garbage collector should rename files rather than deletes them. Default value is
   * {@code false}. It makes sense to change this setting only for debugging and troubleshooting purposes.
   * <p>Mutable at runtime: yes
   *
   ...
   * Returns the minimum age of a {@code Log} file (.xd file) to consider it for cleaning by the database garbage
   * collector. The age of the last (the newest, the rightmost) {@code Log} file is {@code 0}, the age of previous file
   * is {@code 1}, etc. Default value is {@code 2}. The age cannot be less than {@code 1}.
   * <p>Mutable at runtime: yes
   *
   ...
   * <p>Mutable at runtime: yes
   *
   * @return number of milliseconds after which background cleaning cycle can be repeated if the previous one didn't
   * reach required utilization
   */
  ...
  /**
   * Returns {@code true} if database utilization will be computed from scratch before the first cleaning cycle (single
   * run of the database garbage collector) is triggered, i.e. shortly after the database is open. In addition, can be
   * used to compute utilization information at runtime by just modifying the setting value. Default value is
   * {@code false}.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * Set {@code true} if database utilization should be computed from scratch before the first cleaning cycle (single
   * run of the database garbage collector) is triggered, i.e. shortly after the database is open. In addition, can be
   * used to compute utilization information at runtime by just modifying the setting value. Default value is
   * {@code false}.
   * <p>Mutable at runtime: yes
  ...
  /**
   * Returns full path to the file with stored utilization. Is used on creation of an {@linkplain Environment} to update
   * {@code .xd} files' utilization before the first cleaning cycle (single run of the database garbage collector) is
   * triggered. In addition, can be used to reload utilization information at runtime by just modifying the setting
   * value. Format of the stored utilization is expected to be the same as created by the {@code "-d"} option of the
  ...
  /**
   * Sets full path to the file with stored utilization. Is used on creation of an {@linkplain Environment} to update
   * {@code .xd} files' utilization before the first cleaning cycle (single run of the database garbage collector) is
   * triggered. In addition, can be used to reload utilization information at runtime by just modifying the setting
   * value. Format of the stored utilization is expected to be the same as created by the {@code "-d"} option of the
   * {@code Reflect} tool. Default value is empty string.
  ...
  /**
   * Returns {@code true} if the database garbage collector tries to acquire exclusive {@linkplain Transaction} for its
   * purposes. In that case, GC transaction never re-plays. In order to not block background cleaner thread forever,
   * acquisition of exclusive GC transaction is performed with a timeout controlled by the
   * {@linkplain #GC_TRANSACTION_ACQUIRE_TIMEOUT} setting. Default value is {@code true}.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * Sets {@code true} if the database garbage collector should try to acquire exclusive {@linkplain Transaction} for
   * its purposes. In that case, GC transaction never re-plays. In order to not block background cleaner thread forever,
   * acquisition of exclusive GC transaction is performed with a timeout controlled by the
   * {@linkplain #GC_TRANSACTION_ACQUIRE_TIMEOUT} setting. Default value is {@code true}.
   * <p>Mutable at runtime: yes
   *
   * @param useExclusiveTransaction {@code true} if the database garbage collector should try to acquire exclusive
   ...
   * <p>Mutable at runtime: yes
   *
   * @return timeout in milliseconds which is used by the database garbage collector to acquire exclusive
   * {@linkplain Transaction}
   ...
   * <p>Mutable at runtime: yes
   *
   * @param txnAcquireTimeout timeout in milliseconds which is used by the database garbage collector to acquire
  ...
  /**
   * Returns timeout in milliseconds which is used by the database garbage collector to reclaim non-expired data in
   * several files inside single GC {@linkplain Transaction} acquired exclusively.
   * {@linkplain #GC_USE_EXCLUSIVE_TRANSACTION} should be {@code true}. Default value is {@code 500}.
   * <p>Mutable at runtime: yes
   *
   * @return timeout in milliseconds which is used by the database garbage collector to reclaim non-expired data in
   * several files inside single {@linkplain Transaction} acquired exclusively
   * @see #getGcUseExclusiveTransaction()
   * @see #getGcTransactionAcquireTimeout()
   */
  ...
  /**
   * Sets timeout in milliseconds which is used by the database garbage collector to reclaim non-expired data in several
   * files inside single GC {@linkplain Transaction} acquired exclusively. {@linkplain #GC_USE_EXCLUSIVE_TRANSACTION}
   * should be {@code true}. Default value is {@code 500}.
   * <p>Mutable at runtime: yes
   *
   * @param txnTimeout timeout in milliseconds which is used by the database garbage collector to reclaim non-expired
   *                   data in several files inside single {@linkplain Transaction} acquired exclusively
   * @return this {@code EnvironmentConfig} instance
   * @see #setGcUseExclusiveTransaction(boolean)
   * @see #setGcTransactionAcquireTimeout(int)
   */
  ...
  /**
   * Returns the number of milliseconds which deletion of any successfully cleaned {@code Log} file (.xd file) is
   * postponed for. Default value is {@code 5000}.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * Sets the number of milliseconds which deletion of any successfully cleaned {@code Log} file (.xd file) is postponed
   * for. Default value is {@code 5000}.
   * <p>Mutable at runtime: yes
   *
  ...
  /**
   * GC is forced every this number of seconds. Default value is {@code 0} which means that GC is not forced
   * periodically.
   * <p>Mutable at runtime: yes
   */
  ...
  /**
   * Sets GC to be forced every this number of seconds. If the value is zero GC is not forced periodically. Default
   * value is {@code 0}.
   * <p>Mutable at runtime: yes
   */
  ...
  /**
   * If {@linkplain #isManagementEnabled()} then set {@code false} in order to expose operations with JMX managed beans
  ...
  public int getEnvTransactionPermissionTimeout() {
    return (Integer) getSetting(ENV_TRANSACTION_PERMISSION_TIMEOUT);
  }
}
