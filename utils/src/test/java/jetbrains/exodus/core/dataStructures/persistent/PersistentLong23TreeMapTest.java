...
public class PersistentLong23TreeMapTest {
    ...
    public void mutableTreeRandomInsertDeleteTest() {
        ...
        PersistentLong23TreeMap<String> map = new PersistentLong23TreeMap<String>();
    ...
    public void competingWritesTest() {
        PersistentLong23TreeMap<String> tree = new PersistentLong23TreeMap<String>();
        ...
        PersistentLong23TreeMap<String>.ImmutableMap read = tree.getCurrent();
    ...
    public void testSize() {
        ...
        for (int i = 0; i < p.length; i++) {
            if ((i & 15) == 0) {
                if (i > 0) {
                    tree.endWrite();
                    Assert.assertEquals(i, source.getCurrent().size());
                }
        ...
        tree.endWrite();
        Assert.assertEquals(p.length, source.getCurrent().size());
        ...
        for (int i = 0; i < p.length; i++) {
            if ((i & 15) == 0) {
                if (i > 0) {
                    tree.endWrite();
                    Assert.assertEquals(p.length - i, source.getCurrent().size());
                }
                tree = source.beginWrite();
            }
        ...
        tree.endWrite();
        Assert.assertEquals(0, source.getCurrent().size());
    }
