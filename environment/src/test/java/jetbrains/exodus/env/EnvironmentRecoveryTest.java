...
public class EnvironmentRecoveryTest extends EnvironmentTestsBase {
    ...
    public void testLastLoggableIncomplete13() {
        Log log = env.getLog();
        final long fileSize = LogTests.adjustedLogFileSize(
                env.getEnvironmentConfig().getLogFileSize() * 1024,
                env.getEnvironmentConfig().getLogCachePageSize());
        log.beginWrite();
        ...
        env.close();
        writer.openOrCreateBlock(env.getEnvironmentConfig().getLogFileSize() * 1024, 0);
        writer.close();
