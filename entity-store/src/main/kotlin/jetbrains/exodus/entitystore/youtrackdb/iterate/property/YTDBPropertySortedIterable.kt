...
class YTDBPropertySortedIterable(
    txn: YTDBStoreTransaction,
    private val entityType: String,
    private val propertyName: String,
    private val source: YTDBEntityIterable? = null,
    private val ascending: Boolean,
) : YTDBEntityIterableBase(txn) {

    override fun query(): YTDBSelect {
        val order = YTDBOrderByFields(propertyName, ascending)
        return if (source != null) {
            source.query().withOrder(order)
        } else {
            YTDBClassSelect(entityType, order = order)
        }
    }
}
