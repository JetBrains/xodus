...
public class EntityFromLinkSetTests extends EntityStoreTestBase {
    ...
    public void testHistory() {
        ...
        final PersistentEntity i1 = txn.newEntity("Issue");
        final Entity i2 = txn.newEntity("Issue");
        final Entity i3 = txn.newEntity("Issue");
        final Entity i4 = txn.newEntity("Issue");
        final Entity i5 = txn.newEntity("Issue");
        ...
        final int version = i1.getVersion();
        final PersistentEntityId persistentId = new PersistentEntityId(i1.getId(), version);
        final PersistentStoreTransaction snapshot = txn.getSnapshot();
        try {
            i1.newVersion(snapshot);
        } finally {
            snapshot.abort();
        }
        i1.deleteLinks("dup");
        ...
        EntityIteratorWithPropId it = (EntityIteratorWithPropId) historyItem.getLinks(names).iterator();
