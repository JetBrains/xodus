...
class YTDBEntityIterableTest : OTestMixin {
    ...
    fun `property is null`() {
        ...
        // When
        withStoreTx { tx ->
            val issues = YTDBPropertyIsNullIterable(tx, Issues.CLASS, "none")
    ...
    fun `no links`() {
        ...
        // When
        withStoreTx { tx ->
            val issues = YTDBLinkIsNullEntityIterable(tx, Issues.CLASS, Issues.Links.IN_PROJECT)
    ...
    fun `property equals`() {
        ...
        // When
        withStoreTx { tx ->
            val issues = YTDBPropertyEqualIterable(tx, Issues.CLASS, "opca", 300)
    ...
    fun `union two iterables`() {
        ...
        withStoreTx { tx ->
            ...
            val issues = equal1.union(equal2) as YTDBUnionEntityIterable
    ...
    fun `intersect two iterables`() {
        ...
        withStoreTx { tx ->
            ...
            val issues = nameEqual.intersect(priorityEqual) as YTDBIntersectionEntityIterable
    ...
    fun `concat iterables selected by properties`() {
        ...
        withStoreTx { tx ->
            ...
            val concat = issue1.concat(issue2) as YTDBConcatEntityIterable
    ...
    fun `find links`() {
        ...
        withStoreTx { tx ->
            ...
            ) as YTDBLinkOfTypeToEntityIterable
    ...
    fun `concat iterables selected by links`() {
        ...
        withStoreTx { tx ->
            ...
            val concat = issuesOnBoard1.concat(issuesOnBoard2) as YTDBConcatEntityIterable
    ...
    fun distinct() {
        ...
        withStoreTx { tx ->
            ...
            val issuesDistinct = issues.distinct() as YTDBDistinctEntityIterable
    ...
    fun `all minus find by property`() {
        ...
        withStoreTx { tx ->
            ...
            val simpleIssues = issues.minus(complexIssues) as YTDBMinusEntityIterable
    ...
    fun `find by property minus find by property`() {
        ...
        withStoreTx { tx ->
            ...
            val complexUnblockedIssues = complexIssues.minus(blockedIssues) as YTDBMinusEntityIterable
    ...
    fun `find by link minus find by link`() {
        ...
        withStoreTx { tx ->
            val issuesOnBoard1 = tx.findLinks(Issues.CLASS, test.board1, Issues.Links.ON_BOARD)
            val issuesOnBoard2 = tx.findLinks(Issues.CLASS, test.board2, Issues.Links.ON_BOARD)
            val issues = issuesOnBoard1.minus(issuesOnBoard2) as YTDBMinusEntityIterable
    ...
    fun `iterable sort and reverse`() {
        ...
        withStoreTx { tx ->
            val reversedByName =
                tx.sort(Issues.CLASS, "name", true).reverse() as YTDBReversedEntityIterable
    ...
    fun `find issues (as iterable) on boards (as iterable)`() {
        ...
        withStoreTx { tx ->
            ...
            val allIssues = tx.getAll(Issues.CLASS) as YTDBEntityIterableBase
            val issuesOnBoards =
    ...
    fun `sort iterable and get first`() {
        ...
        // When
        withStoreTx { tx ->
            val sortedIssues = tx.sort(Issues.CLASS, "name", true) as YTDBEntityIterable
